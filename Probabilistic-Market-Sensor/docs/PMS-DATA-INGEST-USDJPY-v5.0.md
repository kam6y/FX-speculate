...
- volumeは取得不可のため 0.0 保存とし、volume_missing_flag は後段で付与する（要件 v5.0 6.1/7.1 準拠）
- 取引休止時間帯は欠損判定から除外する（詳細は 3.3）
...
### 3.3 取引休止時間帯（GMOデータ観測値）
2025-07-27〜2026-01-26 の取得データにおいて、連続的に発生する最大ギャップの開始/終了は以下で安定していたため、これを市場クローズとして扱う。
- 週次クローズ（UTC）: 金曜 21:00:00 〜 日曜 21:59:59（次のバーは日曜 22:00:00）
- 週次クローズ（JST）: 土曜 06:00:00 〜 月曜 06:59:59（次のバーは月曜 07:00:00）

上記時間帯に収まるギャップは欠損判定の対象外とする。プロバイダの運用変更があった場合は再計測し、時間帯を更新する。
