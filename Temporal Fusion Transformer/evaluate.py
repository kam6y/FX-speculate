...
出力:
  - artifacts/eval/eval_report.json   : 全メトリクス
  - artifacts/eval/eval_*.png         : 各種可視化グラフ

実行:
    uv run python evaluate.py                            # best 3 ckpt アンサンブル
    uv run python evaluate.py --ckpt path/to/model.ckpt  # 単一モデル
    uv run python evaluate.py --top-k 5                  # top-5 アンサンブル
"""
...
from config import CONFIG, ARTIFACT_DIR, DEVICE, PIN_MEMORY
from model import load_model, find_checkpoints, ensemble_predict_with_actuals
from data import prepare_data, create_datasets
from train import backtest_trading, _direction_accuracy

EVAL_DIR = ARTIFACT_DIR / "eval"
EVAL_DIR.mkdir(parents=True, exist_ok=True)


def _ensemble_predict_with_actuals(
    models: list, dataset, config: dict
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """複数モデルの予測を平均し、actuals と pred_time_idx も返す"""
    dl = dataset.to_dataloader(
        train=False, batch_size=config["BATCH_SIZE"], num_workers=0,
        pin_memory=PIN_MEMORY,
    )
    return ensemble_predict_with_actuals(models, dl)
...
def compute_full_metrics(
    ...
    for h_end, h_name in [(1, "1d"), (2, "2d"), (3, "3d"), (5, "5d"), (10, "10d")]:
        if h_end <= actuals.size(1):
            dir_metrics[f"direction_{h_name}"] = round(_direction_accuracy(q50, actuals, h_end), 4)
...
# 可視化 (9 plots)
# ===================================================================
def plot_equity_curve(preds, actuals, dates, config):
    _, actual_1d, signals, pnl, cost_lr = _extract_1d_pnl(preds, actuals, config)
    cum_pnl = np.cumsum(pnl)
    running_max = np.maximum.accumulate(cum_pnl)
    drawdown = cum_pnl - running_max
    bh_cum = np.cumsum(actual_1d - cost_lr)
    plot_dates = dates[:len(cum_pnl)]

    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(14, 8), height_ratios=[3, 1],
                                     sharex=True, gridspec_kw={"hspace": 0.05})
    ax1.plot(plot_dates, cum_pnl, label="TFT モデル", color="#2196F3", linewidth=1.5)
    ax1.plot(plot_dates, bh_cum, label="Buy & Hold", color="#9E9E9E", linewidth=1, linestyle="--", alpha=0.7)
...
def plot_monthly_returns(preds, actuals, dates, config):
...
def plot_rolling_accuracy(preds, actuals, dates, window=60):
    q_mid = preds.size(2) // 2
    ...
    ax.axhline(np.mean(correct), color="#4CAF50", linewidth=1, linestyle=":", alpha=0.7, label=f"全期間平均 ({np.mean(correct):.1%})")
    ax.fill_between(plot_dates, 0.5, rolling_acc, where=rolling_acc > 0.5, alpha=0.15, color="#4CAF50")
    ax.fill_between(plot_dates, 0.5, rolling_acc, where=rolling_acc < 0.5, alpha=0.15, color="#F44336")
    ax.set_ylabel("方向精度")
...
def plot_prediction_scatter(preds, actuals):
    q_mid = preds.size(2) // 2
    pred_1d = preds[:, 0, q_mid].cpu().numpy()
    actual_1d = actuals[:, 0].cpu().numpy()
    ...
    ax.scatter(actual_1d[correct], pred_1d[correct], alpha=0.4, s=15, c="#4CAF50", label=f"方向一致 ({correct.sum()})")
    ax.scatter(actual_1d[~correct], pred_1d[~correct], alpha=0.4, s=15, c="#F44336", label=f"方向不一致 ({(~correct).sum()})")
...
def plot_quantile_fan(preds, actuals, dates, last_n=100):
    ...
    fig, ax = plt.subplots(figsize=(14, 5))
    ax.fill_between(x, pred_q10, pred_q90, alpha=0.2, color="#2196F3", label="80% 予測区間")
...
def plot_regime_accuracy(preds, actuals, df_test):
    q_mid = preds.size(2) // 2
    ...
    for bar, (_, row) in zip(bars, stats.iterrows()):
        ax.text(bar.get_width() + 0.01, bar.get_y() + bar.get_height() / 2,
                f'{row["accuracy"]:.1%} (n={int(row["count"])})', va="center", fontsize=10)
    ax.set_xlabel("方向精度")
...
def plot_horizon_accuracy(preds, actuals):
    q_mid = preds.size(2) // 2
    q50 = preds[:, :, q_mid]
    horizons, accs = [], []
    ...
    for bar, acc in zip(bars, accs):
        ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + 0.005, f"{acc:.1%}", ha="center", fontsize=9)
...
def plot_pnl_distribution(preds, actuals, config):
    _, _, _, pnl, _ = _extract_1d_pnl(preds, actuals, config)
    fig, ax = plt.subplots(figsize=(10, 5))
    ax.hist(pnl, bins=50, color="#2196F3", alpha=0.7, edgecolor="white")
    ax.axvline(0, color="black", linewidth=1)
    ax.axvline(np.mean(pnl), color="#4CAF50", linewidth=1.5, linestyle="--", label=f"平均: {np.mean(pnl):.5f}")
    ax.axvline(np.median(pnl), color="#FF9800", linewidth=1.5, linestyle=":", label=f"中央値: {np.median(pnl):.5f}")
    win_rate = (pnl > 0).sum() / len(pnl)
    ax.text(0.98, 0.95, f"勝率: {win_rate:.1%}\n取引数: {len(pnl)}\nSharpe: {np.mean(pnl)/max(np.std(pnl),1e-10)*252**0.5:.2f}",
            transform=ax.transAxes, va="top", ha="right", fontsize=10, bbox=dict(boxstyle="round", facecolor="wheat", alpha=0.5))
...
def plot_summary_dashboard(metrics):
    ...
    for bar, val in zip(bars, dir_vals):
        ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + 0.01, f"{val:.1%}", ha="center", fontsize=9)
    ax.set_title("方向精度")
    ax.set_ylim(0, 1)
    ...
    table = ax.table(cellText=table_data, colLabels=["指標", "値"], loc="center", cellLoc="center")
    ...
    ax.axis("off")
    summary_text = (
        f"取引数: {metrics.get('trade_n_trades', 0)}\n"
        f"80% カバレッジ: {metrics.get('quantile_coverage_80', 0):.1%}\n"
        f"MAE: {metrics.get('mae', 0):.4f}\n"
        f"RMSE: {metrics.get('rmse', 0):.4f}"
    )
    ax.text(0.5, 0.5, summary_text, transform=ax.transAxes, fontsize=14, va="center", ha="center",
...
def main():
    ...
    parser.add_argument("--ckpt", type=str, nargs="+", default=None, help="チェックポイントファイルパス (複数可)")
    parser.add_argument("--top-k", type=int, default=3, help="val_loss 上位 k 個でアンサンブル")
    ...
    if args.ckpt:
        ...
        ckpt_paths = find_checkpoints(ckpt_dir, top_k=args.top_k)
        print(f"Selected {len(ckpt_paths)} checkpoints:")
        for p in ckpt_paths:
            print(f"  {p.name}")
    ...
    print("\n=== 4. 予測 ===")
    ens_preds, actuals, pred_time_idx = _ensemble_predict_with_actuals(models, test, CONFIG)
